...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.coli.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.colis.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                <label class="required" for="destinataire_nom">{{ trans('cruds.coli.fields.destinataire_nom') }}</label>
                <input class="form-control {{ $errors->has('destinataire_nom') ? 'is-invalid' : '' }}" type="text" name="destinataire_nom" id="destinataire_nom" value="{{ old('destinataire_nom', '') }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('destinataire_tel') ? 'is-invalid' : '' }}" type="text" name="destinataire_tel" id="destinataire_tel" value="{{ old('destinataire_tel', '') }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control timepicker {{ $errors->has('heure_envoi') ? 'is-invalid' : '' }}" type="text" name="heure_envoi" id="heure_envoi" value="{{ old('heure_envoi') }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control timepicker {{ $errors->has('heure_retrait') ? 'is-invalid' : '' }}" type="text" name="heure_retrait" id="heure_retrait" value="{{ old('heure_retrait') }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('statut') ? 'is-invalid' : '' }}" name="statut" id="statut">
                    ...
                    @foreach(App\Models\Coli::STATUT_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('statut', 'en cours|livré') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('expediteur') ? 'is-invalid' : '' }}" name="expediteur_id" id="expediteur_id" required>
                    @foreach($expediteurs as $id => $entry)
                        <option value="{{ $id }}" {{ old('expediteur_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
